...
-define(SERVICE, 'grpcbox.RouteGuide').
...
get_feature(Ctx, Input, Options) ->
    grpcbox_client:unary(Ctx, <<"/grpcbox.RouteGuide/GetFeature">>, Input, ?DEF(point, feature), Options).
...
list_features(Ctx, Input, Options) ->
    grpcbox_client:stream(Ctx, <<"/grpcbox.RouteGuide/ListFeatures">>, Input, ?DEF(rectangle, feature), Options).
...
record_route(Ctx, Options) ->
    grpcbox_client:stream(Ctx, <<"/grpcbox.RouteGuide/RecordRoute">>, ?DEF(point, route_summary), Options).
...
route_chat(Ctx, Options) ->
    grpcbox_client:stream(Ctx, <<"/grpcbox.RouteGuide/RouteChat">>, ?DEF(route_note, route_note), Options).
